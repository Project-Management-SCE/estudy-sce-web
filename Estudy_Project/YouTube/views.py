...
from YouTube.form import CourseForm ,CreatCourseForm
...
class SearchVideoView(View):
  ...
  def post(self,request):
    ...
    if SEARCH != '':
      ...
      for lst in new_list:
        YouTube.objects.create(index = lst['index'],channelId=lst['channelId'], IdVideo=lst['IdVideo'], profile=lst['profile'], title=lst['title'],thumbnails=lst['thumbnails']).save()
...
class SaveVideoView(View):
  def get(self,request,video_index):
    ...
    return render(request,"save.html", {'form':form_course,'video_index':video_index})  
  ...
  def post(self,request,video_index):
    ...
    if form.is_valid():
      ...
      if Course.objects.filter(department=depart,year=year,semester=semester,kind_of=kind_of).exists():
        ...
        return render(request,"save.html",{'form':form,'all_courses':all_courses,'video_index':video_index})
      else:
        return render(request,"save.html",{"error":"No results have been found",'form':form,'video_index':video_index})
...
class CreateCourseView(View):
  def get(self,request,video_index):
    ...
    return render(request,"save.html",{'folder': folder,'form':form, 'video_index':video_index})
...
class YouTubeVideoView(View):
  def get(self,request,course_id,user_id,video_index):
    return render(request,"YoTubeVideo.html",{'course_id':course_id,'user_id':user_id,'video_index':video_index})

  def post(self,request,course_id,user_id,video_index):
    user = User.objects.get(id=user_id)
    course = Course.objects.get(id=course_id)
    video = YouTube.objects.get(index=video_index)
    YouTubeVideo.objects.create(user=user, course=course,channelId=video.channelId, IdVideo=video.IdVideo, profile=video.profile, title=video.title, thumbnails=video.thumbnails).save()
    print("SUCCESSSSSS")
