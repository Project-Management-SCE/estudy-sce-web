...
class StudentForm(forms.ModelForm):
    ...
    class Meta():
      model = Student
      fields = ('first_name','last_name','Phone','ID','department','year','semester') 
      widgets = {
    ...
    def clean_ID(self):
      ...
      elif (Student.objects.filter(ID=ID).exists() and len(Student.objects.filter(ID=ID)) > 1) or (Lecturer.objects.filter(ID=ID).exists()):
...
class UserUpdate(forms.ModelForm):
    ...
    class Meta():
        ...
        widgets = {
            'email': forms.TextInput(attrs={ 'type':"text" ,'class':"bg-light form-control", }),
        }
...
class LecturerForm(forms.ModelForm):
    ...
    def clean_ID(self):
      ...
      if not ID.isdecimal():
          ...
          raise forms.ValidationError("ID must to contain 9 digits.")
      elif (Student.objects.filter(ID=ID).exists()) or (Lecturer.objects.filter(ID=ID).exists() and len(Lecturer.objects.filter(ID=ID)) > 1):
          raise forms.ValidationError("ID exsits")
      return ID
